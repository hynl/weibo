...
import com.hynl.weibo.hynladapter.StatusCommentAdapter;
import com.hynl.weibo.hynladapter.StatusGridImgsAdapter;
...
import com.hynl.weibo.hynlentity.response.CommentsResponse;
...
public class StatusDetailActivity extends BaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mAccess2Token= AccessTokenKeeper.readAccessToken(StatusDetailActivity.this);
		// 获取intent传入的信息
	    status = (Status) getIntent().getSerializableExtra("status");
		//发送成功后返回直接滚动到评论部分，忽略headerview
...
//		底部菜单栏隐藏
	...
	private void initTab() {
        // 完成两个窗口，同步使用配合显示隐藏；
		// shadow
		shadow_status_detail_tab = findViewById(R.id.status_detail_tab);//布局id里的窗口，
...
//		//需要添加的header 2
	...
	public void onClick(View v) {
		...
		case R.id.titlebar_iv_left:
			...
			break;
		case R.id.iv_image:
//			if(status.getUser().getIdstr().equals(mAccess2Token.getUid())){
//				Intent intent = new Intent(StatusDetailActivity.this, UserInfoActivity.class);
//				intent.putExtra("userName", status.getUser().getScreen_name());
//				startActivity(intent);
//			}else{
//				ActivityInvokeAPI.openMessageListByUid(StatusDetailActivity.this, status.getUser().getIdstr());
//			}
			ToastUtils.showToast(StatusDetailActivity.this, status.getUser().getName(), Toast.LENGTH_SHORT);
			break;
